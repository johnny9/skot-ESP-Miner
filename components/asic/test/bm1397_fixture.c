...
const bm1397_fixture_driver_t bm1397_fixture_driver = {
...
static bm1397_fixture_packet_t packets[MAX_PACKETS];
static size_t packet_count;
static uint8_t queued_response[BM1397_FIXTURE_RESPONSE_SIZE];
...
int SERIAL_send(uint8_t *bytes, int length, bool debug)
{
    ...
    bm1397_fixture_packet_t *packet = &packets[packet_count++];
...
int count_asic_chips(uint16_t count, uint16_t chip_id, int response_length)
{
    ...
    TEST_ASSERT_EQUAL_INT(BM1397_FIXTURE_RESPONSE_SIZE, response_length);
    return count;
}
