#ifndef JOB_PIPELINE_FIXTURE_H
#define JOB_PIPELINE_FIXTURE_H
...
#define JOB_PIPELINE_FIXTURE_MAX_JOBS 8

typedef enum {
    JOB_PIPELINE_FIXTURE_TIMEOUT = 0,
    JOB_PIPELINE_FIXTURE_NOTIFY,
} job_pipeline_fixture_event_type_t;

typedef struct {
    job_pipeline_fixture_event_type_t type;
    uint32_t slot;
} job_pipeline_fixture_event_t;

typedef struct {
    bm_job *jobs[JOB_PIPELINE_FIXTURE_MAX_JOBS];
    size_t job_count;
    uint32_t version_masks[JOB_PIPELINE_FIXTURE_MAX_JOBS];
