...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        // The ASIC component snapshots header bytes and metadata while holding
        // its job lock, so slot reuse during a blocking submit is safe.
        asic_job_t active_job_snapshot;
        if (!ASIC_get_job_snapshot(GLOBAL_STATE, job_id, &active_job_snapshot)) {
            ESP_LOGW(TAG, "Invalid job nonce found, 0x%02X", job_id);
            continue;
        }
        const asic_job_t *active_job = &active_job_snapshot;
        ...
        ESP_LOGI(TAG, "ID: %s, ASIC nr: %d, Core: %d/%d, ver: %08" PRIX32 " Nonce %08" PRIX32 " diff %.1f of %g.", active_job->job_id, asic_result->asic_nr, asic_result->core_id, asic_result->small_core_id, asic_result->rolled_version, asic_result->nonce, nonce_diff, active_job->pool_diff);

        SYSTEM_notify_found_nonce(GLOBAL_STATE, nonce_diff, active_job->nbits);

        scoreboard_add(&GLOBAL_STATE->SYSTEM_MODULE.scoreboard, nonce_diff, active_job->job_id, active_job->extranonce2, active_job->ntime, asic_result->nonce, version_bits);
    }
}
